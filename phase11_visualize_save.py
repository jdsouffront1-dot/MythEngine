...
print("\n--- MythEngine Phase 11.1: Visualization & Summary Export ---")

df = pd.read_csv("data/metrics.csv")
df.columns = [c.lower() for c in df.columns]

if not all(k in df.columns for k in ["cycle","coherence","epsilon"]):
...
plt.title("MythEngine Phase 11.1 - Adaptive Dynamics Visualization")
...
plt.tight_layout()
plt.savefig("data/phase11_visualization.png", dpi=300)
print("Saved visualization to data/phase11_visualization.png")

summary = {
    "mean_coherence": df["coherence"].mean(),
    "var_coherence": df["coherence"].var(),
    "rebounds": len(rebounds)
}
pd.DataFrame([summary]).to_csv("data/phase11_summary.csv", index=False)
print("Saved summary to data/phase11_summary.csv")
